const postDataController = function(){
    ...
    const addCommentToDOMTree = function(username, gender, comment, userId){
        ...
        `<div class="user-comment">
            ...
            <div class="usernameWrapper">
                <a class="username" href="#/profile/${userId}">${username}</a>
            </div>
        </div>
        <div class="comment">
            <p>${comment}</p>
        ...
        let isFirstComment = document.querySelector('.noComments') !== null;
        
        if(isFirstComment){
            let li = document.querySelector('.noComments');
    ...
    const postComment = function(context){
    ...
    const publishComment = async function(context){
        let story = {};
        ...
        story.comments.push({userId, comment: context.params.comment});

        storyService.edit(context.params.id, story);

        let {username, gender} = await userService.getUserDataFromId(userId);

        addCommentToDOMTree(username, gender, context.params.comment, userId);
    }
